...
#   RETORNA UM MENTOR DE ACORDO COM O ID
...
#   RETORNA UM MENTORANDO DE ACORDO COM O ID
...
# RETORNAR  UMA MENTORIA DE ACORDO COM O ID
...
@app.route("/mentor/<int:id_mentor>", methods = ["PUT"])
...
@app.route("/mentorando/<int:id_mentorando>", methods = ["PUT"])
...
@app.route("/mentoria/<int:id_mentoria>", methods = ["PUT"])
def update_mentoria(id_mentoria):
    ...
    if mentoria:
        ...
        return jsonify(mentoria.to_dict())
         ...
         return jsonify({"error": "Mentoria não encontrado"}), 404

# REMOVER UM ÚNICO MENTOR
@app.route("/mentor/<int:id_mentor>", methods = ["DELETE"])
...
@app.route("/mentorando/<int:id_mentorando>", methods = ["DELETE"])
...
@app.route("/mentoria/<int:id_mentoria>", methods = ["DELETE"])
